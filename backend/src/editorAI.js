...
async function editVideo(videoObj) {
...
`;
    ...
    try{
        ...
        // console.log(jsonResult.timeline.tracks[2].clips);
...
const getShotstackId = async (jsonResult) => {
...
const fetchStatus = async (renderId) => {
    const url = `https://api.shotstack.io/edit/stage/render/${renderId}`;

    while (true) { // Будет выполняться, пока цикл не прервётся изнутри
        const response = await fetch(url, { method: 'GET', headers: headers });
        const body = await response.json();

        if (body.response.status === 'done') { // Проверяем, достигнут ли нужный статус
        ...
        await new Promise(resolve => setTimeout(resolve, 10000)); // Задержка перед следующей проверкой
    }
}
